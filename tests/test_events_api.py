...
Covers list, create, retrieve, update, and delete operations with a stubbed service to simulate GoogleCalendarService behavior.
...
- Seeds a test client and valid OAuthCredential using datetime objects for expires_at
- Monkeypatches GoogleCalendarService to return controlled, deterministic responses
- Removes duplicate blueprint registration and corrects indentation for Pylance compatibility
"""
import pytest
import uuid
import datetime
...
def app():
    ...
    with app.app_context():
        ...
        client_obj = Client(
            ...
            google_account_email='g1@example.com'
        )
...
def stub_service(monkeypatch):
    ...
    monkeypatch.setattr(
        GoogleCalendarService, '__init__', lambda self, creds: None
    )
    monkeypatch.setattr(
        GoogleCalendarService, 'list_events', DummyService.list_events
    )
    monkeypatch.setattr(
        GoogleCalendarService, 'get_event', DummyService.get_event
    )
    monkeypatch.setattr(
...
def test_create_event(client):
    ...
    resp = client.post(
        f'/api/clients/{cid}/calendars/cal1/events', json=payload
    )
...
def test_update_event(client):
    ...
    resp = client.put(
        f'/api/clients/{cid}/calendars/cal1/events/evt1', json=payload
    )
    assert resp.status_code == 200
    data = resp.get_json().get('data')
    assert data is not None
...
def test_delete_event(client):
    ...
    cid = client_obj.id
    resp = client.delete(
        f'/api/clients/{cid}/calendars/cal1/events/evt1'
    )
    assert resp.status_code == 200
