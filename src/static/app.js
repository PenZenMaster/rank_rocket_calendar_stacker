...
Description:
Frontend JavaScript for Rank Rocket Calendar Stacker. Handles dashboard, clients, OAuth (Settings) UI/API interactions, including full OAuth Settings CRUD and Event CRUD wiring.
...
Comments:
- Added OAuth Settings CRUD: fetch, display, edit, delete credentials
- Implemented deleteOAuthCredential function for credential deletion
- Enhanced showOAuthModal to support editing existing credentials
- Patched OAuth endpoints wiring
*/
  ...
  try {
    ...
    currentClients.forEach((client) => {
      ...
      row.innerHTML = `
                ...
                <td>
                    <button class="btn btn-sm btn-info" onclick="showEditClientModal(${
                      client.id
                    })">Edit</button>
...
async function showOAuthModal(oauthId = null) {
  ...
  await loadClientsForOAuth();
  if (oauthId) {
    try {
      const resp = await apiCall(`/api/oauth/${oauthId}`);
      const cred = resp.data;
      document.getElementById("oauthClientSelect").value = cred.client_id;
      document.getElementById("googleClientId").value = cred.google_client_id;
      document.getElementById("googleClientSecret").value =
        cred.google_client_secret;
      document.getElementById("oauthScopes").value = Array.isArray(cred.scopes)
        ? cred.scopes.join("\n")
        : "";
    } catch (error) {
      console.error("Failed to load OAuth credential for editing:", error);
    }
  }
  modal.show();
}
...
async function deleteOAuthCredential(oauthId) {
  if (!confirm("Are you sure you want to delete this OAuth credential?"))
    return;
  try {
    const resp = await apiCall(`/api/oauth/${oauthId}`, { method: "DELETE" });
    showAlert(resp.message || "Credential deleted", "warning");
    loadOAuthCredentials();
  } catch (error) {
    console.error("Failed to delete OAuth credential:", error);
  }
}
  ...
  try {
    ...
    (Array.isArray(resp.data) ? resp.data : []).forEach((cred) => {
      ...
      row.innerHTML = `
                ...
                <td>
                    <button class="btn btn-sm btn-primary me-1" onclick="showOAuthModal(${
                      cred.id
                    })">Edit</button>
                    <button class="btn btn-sm btn-danger" onclick="deleteOAuthCredential(${
                      cred.id
                    })">Delete</button>
                </td>
            `;
...
async function loadEvents() {
  ...
  try {
    ...
    let html =
      '<table class="table table-striped"><thead><tr><th>Summary</th><th>Start</th><th>End</th><th>Actions</th></tr></thead><tbody>';
