...
def get_clients():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": True, "data": [client.to_dict() for client in clients]}
            ),
            200,
        )
...
def create_client():
    ...
    try:
        ...
        existing_google_account = Client.query.filter_by(
    ...
    try:
        ...
        if "email" in data:
            ...
            existing_client = Client.query.filter(
                Client.email == data["email"], Client.id != client_id
            ).first()
            if existing_client:
                return (
                    jsonify(
...
def get_client_oauth(client_id):
    ...
    try:
        client = Client.query.get_or_404(client_id)
        oauth_creds = OAuthCredential.query.filter_by(
            client_id=client_id, is_valid=True
        ).first()
        ...
        return jsonify({"success": True, "data": oauth_creds.to_dict()}), 200
...
def get_all_oauth():
    ...
    try:
        ...
        return (
            jsonify(
                {"success": True, "data": [cred.to_dict() for cred in oauth_creds]}
            ),
            200,
        )
    except Exception as e:
...
def create_oauth_credentials():
    ...
    try:
        ...
        existing_oauth = OAuthCredential.query.filter_by(
            client_id=data["client_id"], is_valid=True
        ).first()
        if existing_oauth:
            return (
                jsonify(
...
def update_oauth_credentials(oauth_id):
    ...
    try:
        ...
        if "google_client_secret" in data:
            oauth_creds.google_client_secret = data[
                "google_client_secret"
            ]  # In production, encrypt this
        if "scopes" in data:
...
def authorize_oauth(oauth_id):
    ...
    try:
        ...
        params = {
            ...
            + "api/oauth/callback",  # This would be your callback URL
            "scope": (
                oauth_creds.scopes.replace(",", " ")
                if oauth_creds.scopes
...
def oauth_callback():
    ...
    try:
        ...
        if error:
            return (
                jsonify(
                    {"success": False, "error": f"OAuth authorization failed: {error}"}
                ),
                400,
            )
        ...
        if not oauth_creds:
            return (
                jsonify(
                    {"success": False, "error": "Invalid OAuth credentials reference"}
                ),
                404,
            )
        ...
        if "expires_in" in token_json:
            ...
            oauth_creds.token_expires_at = datetime.utcnow() + timedelta(
