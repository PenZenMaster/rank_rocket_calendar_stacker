...
Comments:
- Removed blueprint template_folder to use app-level templates
- Updated render_template calls to reference templates/calendar subfolder
- Ensured explicit return types and flash handling
"""
...
def _get_service_or_redirect(client_id: int) -> GoogleCalendarService | None:
    ...
    if not creds:
        flash(
            "No Google OAuth credentials found. Connect your account first.",
            "warning",
        )
        return None
...
@calendar_bp.route(
    "/clients/<int:client_id>/calendars/events/new",
    methods=["GET", "POST"],
)
